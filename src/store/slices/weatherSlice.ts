...
const fetchWeatherData = async (city: string): Promise<WeatherData> => {
  ...
  const validateUrl = `https://nominatim.openstreetmap.org/search?format=json&limit=10&q=${encodeURIComponent(
    rawQuery
  )}`;
  ...
  const match = candidates.find((item: any) => {
    ...
    const isAdminBoundary =
      itemClass === "boundary" &&
      (itemType === "administrative" || itemType === "political");
  ...
  const canonicalName = String(match?.display_name || rawQuery)
    .split(",")[0]
    .trim();
